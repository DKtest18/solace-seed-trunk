  ...
  const { data: wishlist, isLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      const { data, error } = await db
        .from('dkai_wishlists')
        .select('id, created_at, product_id, dkai_products:product_id(*)')
        .eq('user_id', user.id)
        .order('created_at', { ascending: false });
      if (error) throw error;
      return (data || []).map((r: any) => ({ ...r, products: r.dkai_products }));
    },
    enabled: !!user,
  });
              ...
              <Card key={item.id} className="hover:shadow-lg transition-shadow">
                <CardHeader>
                  <div className="flex justify-between items-start">
                    ...
                    <Button variant="ghost" size="icon" onClick={() => removeFromWishlist.mutate(product.id)}>
